#' Summary Statistics
#'
#' Calculates the means, variances, skewness and kurtosis for a range of
...
#'   into.
#' @param n_sum_stats Number of summary statistics you want to print. The order
#'   of the statistics is as follows: conditional mean, conditional variance,
#'   conditional skewness, conditional kurtosis.
#'
...
# 1. Calculate the quantiles for a selected variable (same as in
# conditional_densities) 2. Calculate a number of summary statistics for all
# other variables conditioned on the selected variable and print them in a list
# with an entry for each summary statistic.
...
sum_stats <- function(var_name, dataset, n_quantiles = 5, n_sum_stats = 3) {
  ...
  names_var <- names(dataset)
  stats_names <- c(
    "conditional_mean", "conditional_variance", "conditional_skewness",
