...
dependencies via the `install_github()` command from the `devtools` package.
...
We will now walk you through the main working steps you will take when working 
with `GGenemy`.
...
After successfully starting the app, you read in a .csv or .txt file that contains
your data. 
...
Step 3 introduces one of the two core functionalities of `GGenemy`: the calculation of conditional summary statistics. You are able to compute the conditional mean, variance, skewness and kurtosis for all your numeric variables,
